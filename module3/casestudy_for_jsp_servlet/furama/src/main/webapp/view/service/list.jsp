...
<div class="container">
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-light mx-auto" style="width: 80% ">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <form class="form-inline my-2 my-lg-0" method="get">
                ...
                <input class="form-control mr-sm-2" type="text" placeholder="Tìm kiếm" aria-label="Search" name="search">
    ...
    <center>
        <h1>Dịch VỤ</h1>
        <h2>
            <a href="/service?action=create">Thêm Mới Dịch Vụ</a>
        </h2>
    ...
    <table class="table">
        ...
        <tr>
            <td><c:out value="${service.maDichVu}"/></td>
            <td><c:out value="${service.tenDichVu}"/></td>
            <td><c:out value="${service.dienTich}"/></td>
            <td><c:out value="${service.chiPhiThue}"/></td>
            <td><c:out value="${service.soNguoiToiDa}"/></td>
            <td><c:out value="${service.tieuChuanPhong}"/></td>
            <td><c:out value="${service.moTaTienNghiKhac}"/></td>
            <td><c:out value="${service.dienTichHoBoi}"/></td>
            <td><c:out value="${service.soTang}"/></td>

            <c:forEach var="rentType" items="${rentTypeList}">
                <c:if test="${service.maKieuThue == rentType.maKieuThue}">
                    <td><c:out value="${rentType.tenKieuThue}"></c:out></td>
                </c:if>
            </c:forEach>

            <c:forEach var="serviceType" items="${serviceTypeList }">
                <c:if test="${service.maLoaiDichVu == serviceType.maLoaiDichVu}">
                    <td><c:out value="${serviceType.tenLoaiDichVu}"></c:out></td>
                </c:if>
            </c:forEach>
...
<%--                &lt;%&ndash;        <td><c:out value="${customer.gioiTinh}"/></td>&ndash;%&gt;--%>
